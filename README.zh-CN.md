...
代码框保留 macOS 窗口结构，正文使用更大的 Maple Mono / SF Mono 字号与较高字重；Python、JSON 等语法高亮采用 Esther Design System 代码面板的紫、蓝、绿、橙色板。右上角常驻一键复制整段代码按钮，Markdown 代码围栏中的加粗、斜体、链接和行内代码标记保持清晰可读。

行内公式不显示上下边线；独立公式在文档内自动连续编号，并把编号放在公式右侧。
